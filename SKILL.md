---
name: lieflat-charts
description: 一套模板驱动、默认 Mono 的数据可视化 skill，严格从 Lupi、Basics、Glance 与 Interactive gallery 的真实实现生成 HTML 图表；明确需要颜色时从三套彩色预设起步，并支持按用户要求继续调色。
---
...
1. **必须从仓库模板生成。** 每张成品图先在 `catalog.md` 锁定图型编号，再打开对应 gallery 的真实实现：Lupi 使用 `templates/lupi-gallery.html`，基础型使用 `templates/basics-gallery.html`，Glance 使用 `templates/glance-gallery.html`，交互大图使用 `templates/big-*.html`。彩色图仍以这些原版模板为结构正本，`templates/color/` 只用于查配色。
...
6. **按模板渲染并自检**（第零、二、三、八节）。逐项核对成品是否仍能对应到所选 gallery 实现；不能因为改数据而换掉模板的核心几何、编码或动效。只有第六点五节放行时才选彩色预设；库外图型走翻译流程（第六节）。
...
**默认永远是 Mono 灰阶。** 用户没提颜色就不要问、不要提，直接出灰阶。

彩色初稿从三套预设中选择一套，色值在 `color-presets.js`，样张在 `templates/color/`。预设用于快速得到稳定、统一的第一版，不是锁死的色板。用户在生成后要求"这个蓝再深一点"或提供自定义色值时，可以继续调整；调整后必须重新检查对比度、视觉主次和数据语义。
...
1. 用户明确要求（"能不能带点颜色""要蓝色的"）。
2. 用户描述了一个颜色本身承载信息的场景（"三条产品线要分开看"）。

其余一律灰阶。彩色不是升级版，是特定场合的替换件。
...
- **初稿从单一预设起步。** 一张图先使用 `color-presets.js` 的同一套颜色。用户明确要求调整后，可以修改或组合色值，但必须重新检查对比度、视觉主次和数据语义，避免不同颜色误导成新的数据分类。
...
- **发光 / 玻璃拟态 / 3D** → 拒绝，保持现有视觉语法。自定义色值与跨预设调整可以接受，但要按第六点五节重新检查可读性和数据语义。
...
2. 色板是否受控？Mono 中出现任何非 ladder 颜色即返工。走彩色时，初稿是否从 `color-presets.js` 的单一预设起步？如果用户调整过颜色，是否重新检查对比度、视觉主次和数据语义，并在需要时于底注说明颜色维度？
