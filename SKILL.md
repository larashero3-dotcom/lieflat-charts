...
3.1 **两组之内还分主力与后备。** 主力是 **L1–L15 与 F1–F13**，默认从这里出图。后备是 **L16–L20、F14–F17、G19–G24**：只有主力里没有能诚实编码这份数据的模板时才用，且必须写明主力的哪几张、因为什么不适配。理由不得只写“新图更好看”或“更专业”。唯一例外见第 3.2 条。
...
3. **必要时才检查 Glance。** 只有 Lupi 与 Basics 候选全部失败，或用户明确要求 Glance / dashboard / 监控 / 周报 / 三秒快读，才扫描 Glance（G1–G24）。选择 Glance 时记录为什么 Lupi 与 Basics 无法承载，不得只写“Glance 更直观”。用户明确要求地图时跳到 Maps（M1–M2），不把地图混入普通候选。
...
- **固定总额按结清状态拆分（已结清 vs 尚未发生）** → 先看主力 F4 Tick Donut / L14 Hundred Field 能不能承接；只有当「未结清」那一段必须被读成尚未发生、而不是又一个并列类目时，才用后备 G23 Hollow Wedge（第零节 3.1，需写明主力为何不适配）
- **计划 vs 实际的时间序列（今天之后仍是计划）** → 先看主力 F2 Hairline Line / F3 Hairline Area 能不能只画已实现的部分；必须同时画出今天之后的计划、并让读者一眼分清哪一段是记录哪一段是承诺时，才用后备 G24 Ledger Steps（第零节 3.1）
...
用户要的图不在 66 张里（或发来一张参考图）时，**不是做不了，是现场造句**。流程四步，第一步不许跳：
